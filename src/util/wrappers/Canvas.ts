...
export class CanvasWrapper {
  static async blur(image: string, blurSize = 4): Promise<Buffer> {
  ...
  static async gray(ctx: Canvas.CanvasRenderingContext2D, image: Canvas.Image): Promise<Canvas.CanvasRenderingContext2D> {
  ...
  static async invert(image: string): Promise<Buffer> {
    const img = await Canvas.loadImage(image);
    const canvas = Canvas.createCanvas(img.width, img.height);
    const ctx = canvas.getContext('2d');
  ...
  static async pixel(image: string, pixelSize = 5): Promise<Buffer> {
    const img = await Canvas.loadImage(image);
    const canvas = Canvas.createCanvas(img.width, img.height);
    const ctx = canvas.getContext('2d');
